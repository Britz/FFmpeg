...
int save_PPS(H264Context *h, PPS* pps, uint pps_id, uint8_t activate_it){
	...
	if(h_base->pps_buffers[pps_id]){
		av_free(h_base->pps_buffers[pps_id]);
		ret = 1;
	}
	h_base->pps_buffers[pps_id] = pps;
	if(activate_it){
		int i;
		h->pps = *pps;
...
int save_SPS(H264Context *h, SPS* sps, uint8_t activate_it){
	...
	if(h->nal_unit_type == NAL_SUB_SPS){
		sps->is_sub_sps = 1;
		if(h_base->sub_sps_buffers[sps->id]){
			av_free(h_base->sub_sps_buffers[sps->id]);
			ret = 1;
		}
		h_base->sub_sps_buffers[sps->id] = sps;
		...
		sps->is_sub_sps = 0;
		if(h_base->sps_buffers[sps->id]){
			av_free(h_base->sps_buffers[sps->id]);
			ret = 1;
		}
		h_base->sps_buffers[sps->id] = sps;
	}else{
		return -1;
	}
	// activate SPS
...
/* == FFmpeg specific things, not in the specification == */
	// set present flag
	h->prefix_nal_present = h0->prefix_nal_present = 1;
	// copy header prefix NAL unit specific values
	// from main context to thread context
	h->nal_ref_idc  = h0->nal_ref_idc,
	h->idr_flag  = h0->idr_flag,
...
//					break;
//				}
//				if(h_base->inter_ref_list[h->sps.anchor_ref[list][h->view_id][i]]){
//					h->ref_list[list][index++] = *h_base->inter_ref_list[h->sps.anchor_ref[list][h->view_id][i]];
//				}
//			}
//		}
		...
		if (get_bits1(&s->gb)) { //ref_pic_list_modification_flag_lX, X=list
			...
			int view_idx_pred = -1; /* picViewIdxLXPred  X=list */
					...
					if (i < 0) {
						...
						int nIdx = index+1;
			 			...
			 			for(i=index; i+1<h->ref_count[list]; i++){
                            if(ref->long_ref == h->ref_list[list][i].long_ref && ref->pic_id == h->ref_list[list][i].pic_id && ref->view_id ==  h->ref_list[list][i].view_id)
                                break;
			 			}
						//i = index-h->inter_ref_count;
						...
						for( i = index+1; i <= h->ref_count[list]; i++ ){
							if(    (h->ref_list[list][i].view_id  != ref->view_id)
								|| ( (modification_type == 1) && (h->ref_list[list][i].pic_id 	!= ref->pic_id))
								|| ( (modification_type == 2) && (h->ref_list[list][i].long_ref != ref->long_ref))
								|| ( (modification_type == 3) && (h->ref_list[list][i].poc 		!= ref->poc))){
...
int ff_h264_mvc_deploy_nal_header(H264Context *h) {
	...
	if(  h->is_mvc && (h->nal_unit_type == NAL_SLICE
		...
		if (h->prefix_nal_present) {
			// reset prefix flag
			if(h->non_idr_flag == 0 && h->nal_unit_type != NAL_IDR_SLICE){
	...
	if(h->prefix_nal_present || h->nal_unit_type == NAL_EXT_SLICE){
		h->idr_flag = !h->non_idr_flag;
	}
	// do some bit stream validation and error parsing
	if(  h->is_mvc && (h->nal_unit_type == NAL_SLICE
					|| h->nal_unit_type == NAL_IDR_SLICE
					|| h->nal_unit_type == NAL_EXT_SLICE) ){
